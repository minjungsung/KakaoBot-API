...
def create_item(sender, room, item_name):
    ...
    existed_item = (
        ...
        .filter(
            and_(
                enhancement_game.room == room,
                enhancement_game.item_name == item_name,
                enhancement_game.user == sender,
            )
        )
        .all()
    )
    ...
    if not existed_item:
        ...
        # 새 아이템 생성
        new_item = enhancement_game(
            user=sender, item_name=item_name, room=room, item_level=after_level
        )
...
"""
        # 히스토리 저장
        new_history = enhancement_history(
            user=sender,
            item_name=item_name,
            room=room,
            before_level=0,
...
"""
        ...
        for index, item in enumerate(existed_item):
            res = (
                res
                + f"{str(index+1)}. [{item.item_name}](Lv.{item.item_level}) - {item.user}\n"
            )
    else:
        res = "순위 내역이 없습니다."
    return res.strip()
...
def get_week_number():
    ...
    first_day_of_week = first_day_of_month - timedelta(
...
def get_my_record(sender, room):
    ...
    total_level_up = (
        ...
        .with_entities(func.sum(enhancement_history.change_level))
        .scalar()
    )
...
def save_guiness():
    ...
    for room in rooms:
        ...
        if best_item_this_week:
            ...
            new_guiness = enhancement_guiness(
                user=item.user,
...
def getRooms():
    ...
    rooms = db.session.query(enhancement_game.room).distinct().all()
    return rooms
...
"""
        ...
        for index, item in enumerate(grave_items):
            res = (
                res
                + f"{str(index+1)}. {item.item_name} (Lv.{item.before_level}) - {item.user}\n"
            )
    else:
        res = "데이터가 존재하지 않습니다."
    return res
